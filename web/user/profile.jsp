...
<body>
    ...
    <% if (session.getAttribute("editSuccess") != null && session.getAttribute("editSuccess").equals("true")) { %>
        ...
        <script>
            // Update URL in browser history to remove updateProfile from the URL
            if (window.location.href.indexOf('updateProfile') > -1) {
                window.history.replaceState({}, document.title, '${pageContext.request.contextPath}/user/profile.jsp');
            }
        </script>
    <% } %>
                ...
                <div class="form-content">
                    <div class="left">
                        ...
                        <div class="info">
                            ...
                            <input type="text" id="name" name="name" value="${not empty param.name ? param.name : user.name}">
                        </div>

                        <div class="info">
                            <label for="email">Email:</label>
                            <input type="email" id="email" name="email" value="${not empty param.email ? param.email : user.email}">
                        </div>

                        <div class="info">
                            <label for="phone">Phone:</label>
                            <input type="tel" id="phone" name="phone" value="${not empty param.phone ? param.phone : user.contact}">
                        </div>
                    </div>
